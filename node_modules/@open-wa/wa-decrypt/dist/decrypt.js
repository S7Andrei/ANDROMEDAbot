...
exports.bleachMessage = exports.decryptMedia = exports.MissingCriticalDataError = exports.mediaTypes = void 0;
...
var MissingCriticalDataError = (function (_super) {
...
var decryptMedia = function (message, useragentOverride) { return __awaiter(void 0, void 0, void 0, function () {
    var options, missingProps, haventGottenImageYet, res, error_1, buff;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                message = message;
                if (!message.mediaKey)
                    missingProps.push('mediaKey');
                if (!message.filehash)
                    missingProps.push('filehash');
                if (!message.mimetype)
                    missingProps.push('mimetype');
                if (!message.type)
                    missingProps.push('type');
                if (!message.size)
                    missingProps.push('size');
                if (!message || !message.mediaKey || !message.filehash || !message.mimetype || !message.type || !message.size) {
                    if (missingProps.length == 1 && missingProps[0] === "size") {
                        if (!nonSizeTypes.includes(message.type))
                            console.warn("@open-wa/wa-decrypt - WARN: size property is missing. File will fail an integrity check.");
                    }
                    else
                        throw new MissingCriticalDataError("Message is missing critical data: ".concat(missingProps.join(', ')));
                }
            ...
            case 2:
                ...
                return [4, axios_1.default.get(message.deprecatedMms3Url.trim(), options)];
            case 3:
                res = _a.sent();
            ...
            case 10:
                ...
                return [2, magix(buff, message.mediaKey, message.type, message.size, message.mimetype)];
        }
    });
}); };
...
var magix = function (fileData, mediaKeyBase64, mediaType, expectedSize, mimetype) {
    var encodedHex = fileData.toString('hex');
    var encodedBytes = hexToBytes(encodedHex);
    var mediaKeyBytes = base64ToBytes(mediaKeyBase64);
    var info = "WhatsApp ".concat(exports.mediaTypes[mediaType.toUpperCase()] || exports.mediaTypes[Object.keys(exports.mediaTypes).filter(function (type) { return mimetype.includes(type.toLowerCase()); })[0]], " Keys");
    var hash = 'sha256';
...
var fixPadding = function (data, expectedSize) {
    ...
    if (padding > 0) {
        if ((expectedSize + padding) == data.length) {
            ...
            var arr = new Uint16Array(padding).map(function () { return padding; });
